...
def run_neural_experiment(config: Dict) -> Dict:
    ...
    pipeline = NeuralCGSEPipeline(config)
    train_summary = pipeline.fit(train_examples)
...
def run_neural_smoke(config: Dict) -> Dict:
    smoke_config = {
        **config,
        "output_dir": "outputs/neural_smoke",
        "data": {
            **config["data"],
    ...
    return run_neural_experiment(smoke_config)
